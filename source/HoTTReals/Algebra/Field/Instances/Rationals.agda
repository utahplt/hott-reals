module HoTTReals.Algebra.Field.Instances.Rationals where
...
numeratorZero→rationalZero (a , b) p = eq/ ((a , b)) ((0 , 1)) q
  where
...
-- TODO: How to do this without fancy open magic?
open CommRingStr (ℚCommRing .snd)
open Units        ℚCommRing

hasInverseℚ : (x : ℚ) → ¬ x ≡ 0 → Σ[ y ∈ ℚ ] x ℚ.· y ≡ 1
hasInverseℚ = elimProp
  (λ x → isPropΠ (λ _ → inverseUniqueness x))
  (λ u p → r u p , q u p)
  where
  r : (u : ℤ × ℕ₊₁) → ¬ [ u ] ≡ 0 → ℚ 
  r (ℤ.pos zero , b) p =
    Empty.rec (p (numeratorZero→rationalZero ((ℤ.pos zero , b)) refl))
